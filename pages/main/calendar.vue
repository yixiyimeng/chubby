	...
	<view class='all'>
		...
		<view  class="myDateTable">
			<view v-for="(item,j) in days" :key="j" class='dateCell'>
				...
				<view v-else>
					<view>
						<view class='cell greenColor bgWhite' style="color: #FF6D89;background-color: rgba(255,255,255,.7);">
							<text>{{item.date}}</text>
						</view>
		...
		<view class="btn-row">
			<view class="btn2" @click="goMain()">开始训练</view>
		</view>
	</view>
</template>
			...
			getscore(){
				uni.request({
					...
					data: {
						...
						scoretime:this.cur_year+"-"+this.cur_month+'-'+1
						
					},
					success: (res) => {
						let row = res.data.rows;
						if(row > 0){
							this.score = row[0].score;
						}else{
							this.score = '';
						}
					}
...
<style>
	...
	.myDateTable {
		...
		padding: 2vw;
		border-radius: 10px;
		background: linear-gradient(#FF6D89, #FFAF75);
	}
